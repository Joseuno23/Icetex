...
class UploadHandler {
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null, $path = false, $folder) {
        $this->response = array();
        $this->path = $path;
...
//        $name = $this->trim_file_name($file_path, $name, $size, $type, $error,
        ...
        $name = $this->trim_file_name($file_path, rand(1, 1000) . time(), $size, $type, $error, $index, $content_range);
    ...
    protected function get_scaled_image_file_paths($file_name, $version) {
        ...
        if (!empty($version)) {
            ...
            if ($type == 'psd' || $type == 'tif' || $type == 'ai') {
                $name = explode('.', $file_name);
                $new_file_path = $version_dir . '/' . $name[0] . '.' . $changeFileType;
            } else {
    ...
    protected function imagick_create_scaled_image($file_name, $version, $options) {
        ...
        } else if ($image->getImageFormat() === 'psd' || $image->getImageFormat() === 'tif' || $image->getImageFormat() === 'ai') {
    ...
    protected function create_image_psd($file_path, $file, $fileType, $version) {
        $saveFileType = "jpg";
        
        $version_dir = $this->get_upload_path(null, $version);
        if (!is_dir($version_dir.'thumbnail/')) {
            mkdir($version_dir.'thumbnail/', $this->options['mkdir_mode'], true);
        }
        
        $image = new \Imagick();
        $image->readimage($file_path);
        if ($fileType == "psd" || $fileType == "tif" || $fileType == "ai") {
            $image->setIteratorIndex(0);
        }
        ...
        $newName = str_replace($fileType, $saveFileType, $file->name);
        
        $paht = str_replace($file->name, "thumbnail/".$newName, $file_path);
        
        $image->writeImage($paht);
        
        return $newName;
        
    }

    protected function handle_image_file_other($file_path, $file, $fileType) {
        $failed_versions = array();
        foreach ($this->options['image_versions'] as $version => $options) {
//            if ($this->create_scaled_image($file->name, $version, $options)) {
            $newName = $this->create_image_psd($file_path, $file, $fileType, $version);
            if (!empty($version)) {
                $file->{$version . 'Url'} = $this->get_download_url($newName, $version);
            } else {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($file_size === $file->size) {
                ...
                if ($this->is_valid_image_file($file_path)) {
                    if ($type == 'psd' || $type == 'tif' || $type == 'ai') {
                        $this->handle_image_file_other($file_path, $file, $type);
                    } else {
